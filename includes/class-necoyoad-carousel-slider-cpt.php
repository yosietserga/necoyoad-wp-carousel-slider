 ...
 * @subpackage Carousel_Slider_CPT
 *
 */
...
if ( ! class_exists( 'Carousel_Slider_CPT' ) ) {
	...
	class Carousel_Slider_CPT {
	     ...
	     */
	    public $textdomain = 'mercadopago-cpt';
...
$product = new Carousel_Slider_CPT('cs_product', [
		'labels'             => [
			'name'               => _x( 'CS Productos', 'post type general name', 'text-domain' ),
			'singular_name'      => _x( 'CS Producto', 'post type singular name', 'text-domain' ),
			'menu_name'          => _x( 'CS Productos', 'admin menu', 'text-domain' ),
			'add_new'            => _x( 'Añadir nuevo', 'cs_producto', 'text-domain' ),
		...
		'show_in_menu'       => true,
		'query_var'          => true,
		'rewrite'            => array( 'slug' => 'cs_producto' ),
...
$product->register_taxonomy([
	'taxonomy_name' => 'type',
	'singular' => 'CS Categoría',
	'plural' => 'CS Categorías',
	'slug' => 'cs_categoria',
	'rewrite'            => array( 'slug' => 'cs_categoria' ),
	'hierarchical'       => true,
    'show_ui'            => true,
    'query_var'          => true
]);
...
function cs_product_settings_callback( $post ) {
	wp_nonce_field( 'cs_product_nonce', '_cscpt_nonce' );
	$_price         = get_post_meta( $post->ID, '_price', true );
	$_price         = (float)$_price ? $_price : 0;
	$_image         = get_post_meta( $post->ID, '_image', true );
	$_url           = get_post_meta( $post->ID, '_url', true );
	?>

    <p>
        <label for="_price">
            <strong><?php esc_html_e( 'Price', 'necoyoad-carousel-slider' ); ?></strong>
        </label>
        <input type="text" name="cs_product[_price]" id="_price" class="small-text"
               value="<?php echo $_price; ?>">
    </p>
    <p>
        <label for="_image">
            <strong><?php esc_html_e( 'External Image', 'necoyoad-carousel-slider' ); ?></strong>
        </label>
        <input type="text" name="cs_product[_image]" id="_image" value="<?php echo $_image; ?>">
    </p>
    <p>
        <label for="_url">
            <strong><?php esc_html_e( 'External Image', 'necoyoad-carousel-slider' ); ?></strong>
        </label>
        <input type="text" name="cs_product[_url]" id="_url" value="<?php echo $_url; ?>">
    </p>
	<?php
}

function add_cpt_meta_boxes() {
	add_meta_box(
		"necoyoad-carousel-slider-csproduct-settings",
		"Settings",
		'cs_product_settings_callback',
		'cs_product',
		"side",
		"low"
	);
...
function save_cpt_meta_box( $post_id ) {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Check if nonce is set.
	if ( ! isset( $_POST['_cscpt_nonce'], $_POST['cs_product'] ) ) {
		return;
	}
	// Check if nonce is valid.
	if ( ! wp_verify_nonce( $_POST['_cscpt_nonce'], 'cs_product_nonce' ) ) {
		return;
	}
	...
	foreach ( $_POST['cs_product'] as $key => $val ) {
		if ( is_array( $val ) ) {
			$val = implode( ',', $val );
		}
...
add_action( 'add_meta_boxes', 'add_cpt_meta_boxes' );
add_action( 'save_post', 'save_cpt_meta_box', 1, 2 );