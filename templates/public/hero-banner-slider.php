...
?>
<div class="necoyoad-carousel-slider-outer necoyoad-carousel-slider-outer-contents necoyoad-carousel-slider-outer-<?php echo $id; ?>">
    ...
    <div <?php echo $slide_options; ?> data-animation="<?php echo $content_animation; ?>">
		...
		foreach ( $content_sliders as $slide_id => $slide ):
			...
			if ( $_link_type == 'full' && carousel_slider_is_url( $_slide_link ) ) {
				$html .= '<a class="necoyoad-carousel-slider-hero__cell hero__cell-' . $slide_id . '" style="' . $_cell_style . '" href="' . $_slide_link . '" target="' . $_link_target . '">';
			} else {
				$html .= '<div class="necoyoad-carousel-slider-hero__cell hero__cell-' . $slide_id . '" style="' . $_cell_style . '">';
			}
			...
			// Background class
			$_slide_bg_class = 'necoyoad-carousel-slider-hero__cell__background';

			if ( 'zoom-in' == $_ken_burns_effect ) {
				$_slide_bg_class .= ' necoyoad-carousel-slider-hero-ken-in';
			} elseif ( 'zoom-out' == $_ken_burns_effect ) {
				$_slide_bg_class .= ' necoyoad-carousel-slider-hero-ken-out';
			}
			...
			$_cell_inner_class  = 'necoyoad-carousel-slider-hero__cell__inner necoyoad-carousel-slider--h-position-center';
			if ( $_content_alignment == 'left' ) {
				$_cell_inner_class .= ' necoyoad-carousel-slider--v-position-middle necoyoad-carousel-slider--text-left';
			} elseif ( $_content_alignment == 'right' ) {
				$_cell_inner_class .= ' necoyoad-carousel-slider--v-position-middle necoyoad-carousel-slider--text-right';
			} else {
				$_cell_inner_class .= ' necoyoad-carousel-slider--v-position-middle necoyoad-carousel-slider--text-center';
			}
			...
			if ( ! empty( $_bg_overlay ) ) {
				...
				$html .= '<div class="necoyoad-carousel-slider-hero__cell__background_overlay" style="' . $_bg_overlay_style . '"></div>';
			}

			$_content_style = '';
			$_content_style .= isset( $settings['content_width'] ) ? 'max-width: ' . $settings['content_width'] . ';' : '850px;';

			$html .= '<div class="necoyoad-carousel-slider-hero__cell__content" style="' . $_content_style . '">';

			// Slide Heading
			$_slide_heading = isset( $slide['slide_heading'] ) ? $slide['slide_heading'] : '';

			$html .= '<div class="necoyoad-carousel-slider-hero__cell__heading">';
			$html .= wp_kses_post( $_slide_heading );
			$html .= '</div>'; // .necoyoad-carousel-slider-hero__cell__heading

			$_slide_description = isset( $slide['slide_description'] ) ? $slide['slide_description'] : '';

			$html .= '<div class="necoyoad-carousel-slider-hero__cell__description">';
			$html .= wp_kses_post( $_slide_description );
			$html .= '</div>'; // .necoyoad-carousel-slider-hero__cell__content

			// Buttons
			if ( $_link_type == 'button' ) {
				$html .= '<div class="necoyoad-carousel-slider-hero__cell__buttons">';
				...
				if ( carousel_slider_is_url( $_btn_1_url ) ) {
					...
					$html .= '<span class="necoyoad-carousel-slider-hero__cell__button__one">';
				...
				if ( carousel_slider_is_url( $_btn_2_url ) ) {
					...
					$html .= '<span class="necoyoad-carousel-slider-hero__cell__button__two">';
					$html .= '<a class="' . $_btn_2_class . '" href="' . $_btn_2_url . '" target="' . $_btn_2_target . '">' . esc_attr( $_btn_2_text ) . "</a>";
					$html .= '</span>';
				}

				$html .= '</div>'; // .necoyoad-carousel-slider-hero__cell__button
			}

			$html .= '</div>'; // .necoyoad-carousel-slider-hero__cell__content
			$html .= '</div>'; // .necoyoad-carousel-slider-hero__cell__inner

			if ( $_link_type == 'full' && carousel_slider_is_url( $_slide_link ) ) {
				$html .= '</a>'; // .necoyoad-carousel-slider-hero__cell
			} else {
				$html .= '</div>'; // .necoyoad-carousel-slider-hero__cell
			}
