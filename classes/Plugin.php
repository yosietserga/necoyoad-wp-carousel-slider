...
class Plugin {
	...
	public static function init() {
		if ( is_null( self::$instance ) ) {
			...
			add_action( 'in_plugin_update_message-necoyoad-carousel-slider/necoyoad-carousel-slider.php',
	...
	public function in_plugin_update_message( array $plugin_data ) {
		...
		if ( version_compare( $current_version_array[0], $new_version_array[0], '<' ) ) {
			...
			$html .= sprintf( __( "<strong>%s</strong> version <strong>%s</strong> is a major update.", 'necoyoad-carousel-slider' ), $plugin_data['Title'], $new_version );
			$html .= '</div>';
			$html .= '<div class="cs_plugin_upgrade_notice__description">';
			$html .= __( 'We made a lot of major changes to this version.', 'necoyoad-carousel-slider' ) . ' ';
			$html .= __( 'We believe that all functionality will remain same after update (remember to refresh you cache plugin).', 'necoyoad-carousel-slider' ) . ' ';
			$html .= __( 'Still make sure that you took a backup so you can role back if anything happen wrong to you.', 'necoyoad-carousel-slider' );
			$html .= '</div>';
