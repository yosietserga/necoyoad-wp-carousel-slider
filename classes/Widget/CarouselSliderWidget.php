...
class CarouselSliderWidget extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description' => __( 'The easiest way to create image, video, post and WooCommerce product carousel.', 'necoyoad-carousel-slider' ),
		);
		parent::__construct( 'widget_carousel_slider', __( 'Carousel Slider', 'necoyoad-carousel-slider' ), $widget_ops );
	}
	...
	public function form( $instance ) {
		...
		if ( count( $carousels ) > 0 ) {
			...
			printf( '<p><label for="%1$s">%2$s</label>', $this->get_field_id( 'title' ), __( 'Title (optional):', 'necoyoad-carousel-slider' ) );
			printf( '<input type="text" class="widefat" id="%1$s" name="%2$s" value="%3$s" /></p>', $this->get_field_id( 'title' ), $this->get_field_name( 'title' ), $title );

			printf( '<p><label>%s</label>', __( 'Choose Slide', 'necoyoad-carousel-slider' ) );
			...
			printf( '<p>%1$s <a href="' . admin_url( 'post-new.php?post_type=carousels' ) . '">%3$s</a> %2$s</p>',
				__( 'You did not add any carousel slider yet.', 'necoyoad-carousel-slider' ),
				__( 'to create a new carousel slider now.', 'necoyoad-carousel-slider' ),
				__( 'click here', 'necoyoad-carousel-slider' )
			);
		}
	}
